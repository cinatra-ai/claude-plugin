...
- **Technical writing** — write and check technical content against Simplified
  Technical English (ASD-STE100): the checkable rules for procedures and
  descriptions, a rewrite path, and a lint report that names the rule, the
...
| `extension-conventions` | Conventions for authoring, pinning, and integrating a Cinatra extension: one repo per extension, the five kinds (workflow scheduled for removal), `cinatra create-extension`, the `package.json#cinatra` manifest shape per kind, the mandatory connector `cinatra/config.json` access declaration, the lock-pin choreography. | You're building or integrating a Cinatra extension and need the cross-kind conventions. |
| `extension-authoring` | The extension development lifecycle: discover (reuse-before-new), collect scaffold inputs (asking for anything missing), scaffold with the published `cinatra` CLI, wire the new repository baseline (gated release ref + its approval Environment, the full gate-caller set, the eslint/vendored-preset lint surface, org enrollment), route payload authoring to the kind specialist, validate after every change, PR/CI, release-readiness handoff (never releases). | You're creating or developing a Cinatra extension end to end. |
| `connector-authoring` | Connector specialist: the mandatory `cinatra/config.json` access scope (lowercase tokens, `default` XOR `only`, protected slugs), UI surfaces (`schema-config` vs `bundled-react`), registration-only `register(ctx)` with type-only SDK imports, least-privilege host ports, outbound-URL allowlisting before and after the fetch, one canonical key per stored record, extraction-cutover ownership and parity, the migrations decision, current fleet archetypes. | You're authoring or reviewing a Cinatra connector. |
...
| `simplified-technical-english` | Author aid + linter for Simplified Technical English (ASD-STE100): the checkable rules (20-word instructions / 25-word sentences, active voice with a named actor, one instruction per sentence, approved verb forms, 3-word noun clusters, no omitted sentence parts, one word / one meaning, safety statements that open with a command, vertical lists), the Technical Name / Technical Verb exception, and a finding format that pairs every violation with a concrete fix. Ships the rules and the method only — the approved-word dictionary stays with ASD. | Writing or reviewing technical documentation, procedures or safety text that must read clearly, translate cleanly, or satisfy an S1000D / ATA i2200 STE requirement. |
...
| `ensure` | The SHARED detect → consent → apply path: READ-ONLY by default (reports the tool's status and, if action is needed, the exact fix command — never installs); `--apply` runs the fix for that one tool and re-verifies. Known tools: `shadcn-skill`. | `--tool <id>` `[--apply]` `[--home d]` `[--codex-home d]` `[--force]` `[--json]` | `node $CLAUDE_PLUGIN_ROOT/bin/dev-tools.cjs ensure --tool shadcn-skill --json` |
