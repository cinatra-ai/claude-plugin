---
name: doctor
description: "Read-only check that a contributor machine is correctly set up: the toolchain (gh, Codex, a GSD install, node/pnpm, Docker, git hooks), toolchain currency, and the global Claude baseline. Reports actionable fixes, never applies them. A thin entry point into the doctor skill so the diagnostic keeps a dedicated / slash command now that skills are hidden from the picker."
argument-hint: "[--json]"
...
is a thin entry point — the full check lives in the **`doctor`** skill
(which is `user-invocable: false`, hidden from the `/` picker, so this command
is its dedicated slash entry).

1. Invoke the **`doctor`** skill. It owns the whole read-only probe:
